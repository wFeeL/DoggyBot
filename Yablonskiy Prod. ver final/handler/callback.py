...
from env import PartnerForm, dp, bot, oferta_url, webapp_url, support_username
import db
from yoomoney import check_invoice, create_invoice
from handler.message import admin_panel
...
async def callback_handler(c: CallbackQuery, state: FSMContext):
    ...
    catigories_markup = types.InlineKeyboardMarkup(inline_keyboard=[[
...
<b>{"🔋" if subscription else "🪫"} Подписка:</b> {"<code>У вас нет подписки</code>" if not subscription else datetime.fromtimestamp(subscription["end_date"]).strftime('%d %B %H:%M')}
        ...
        choose_category_keyboard = [[types.InlineKeyboardButton(text=f"{category["category_name"]}", callback_data=f"category:{category["category_id"]}")] for category in categories]
        ...
        if not message.photo:
            ...
            await bot.send_message(c.from_user.id, """🛍 <b>Категории:</b>""", parse_mode="html", reply_markup=choose_category_markup)
...
{"Ознакомиться с ассортиментом\n" + partner["partner_url"] + "\n" if partner["partner_url"] else ""}<blockquote>{partner["partner_legacy_text"]}</blockquote>"""
            if (len(answer_text + text) >= 842 and len(multiple) == 0) or (len(answer_text + text) >= 3914 and len(multiple) > 0):
...
Партнер может потребовать от Вас ваш индивидуальный промокод из вашего личного кабинета!</blockquote>"""
        ...
        catigories_markup_ = catigories_markup if len(multiple) == 1 else None
            
        if pathlib.Path(f"img/{category_id}.png").is_file():
            await message.edit_media(types.InputMediaPhoto(media=types.FSInputFile(f"img/{category_id}.png", "category_image.png"), caption=answer_text, parse_mode="html"), reply_markup=catigories_markup_)
        else:
            await message.edit_text(text=answer_text, parse_mode="html", reply_markup=catigories_markup_)

        for answer_text in multiple:
            catigories_markup_ = catigories_markup if answer_text == multiple[-1] else None
        ...
        if subscription is None:
            ...
            if user_profile["full_name"]:
                keyboard.append([types.InlineKeyboardButton(text="💳 Оформить подписку", callback_data=f"create_invoice:{settings["length"]}:{settings["price"]}")])
            else:
...
<b>{"🔋" if subscription else "🪫"} Подписка:</b> {"<code>У вас нет подписки</code>" if not subscription else datetime.fromtimestamp(subscription["end_date"]).strftime('%d %B %H:%M')}
        ...
        pay_subscription_markup = types.InlineKeyboardMarkup(inline_keyboard=[
            ...
            types.InlineKeyboardButton(text="🔄 Проверить", callback_data=f"check_invoice:{invoice_id}:{price}:{length}")]
        ])
...
<i>Если возникли какие-либо проблемы писать @{support_username}</i></blockquote>""", parse_mode="html", reply_markup=pay_subscription_markup)
        ...
        if user["level"] > 1 or partner["owner_user_id"] == c.from_user.id: 
            ...
            await message.delete()
            await bot.send_message(user_["user_id"], f"✅ <b>Ваш промокод списан у партнёра <code>{partner["partner_name"]}</code></b>", parse_mode="html")
        ...
        await message.delete()
        await message.answer("👇 <b>Для заполнения формы нажмите на кнопку ниже.</b>", reply_markup=types.ReplyKeyboardMarkup(keyboard=[
            [types.KeyboardButton(text="🪪 Заполнить форму", web_app=types.WebAppInfo(url=webapp_url))]
...
Подписок за год: <code>{len(orders_year)} / {sum([order["price"] for order in orders_year])}₽</code>""", parse_mode="html", reply_markup=admin_markup)
            ...
            partner_buttons = [
                [types.InlineKeyboardButton(text=f"{partner['partner_name']} (ID: {partner['partner_id']})", callback_data=f"partner:{partner['partner_id']}")]
                for partner in partners
            ]
            navigation_buttons = []
            if page > 1:
                navigation_buttons.append(types.InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin:partners:{page - 1}"))
            if page < total_pages:
                navigation_buttons.append(types.InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"admin:partners:{page + 1}"))
            ...
            user_buttons = [
                [types.InlineKeyboardButton(text=f"{user['full_name']} (ID: {user['user_id']})", callback_data=f"user:{user['user_id']}")]
                for user in users
            ]
            navigation_buttons = []
            if page > 1:
                navigation_buttons.append(types.InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin:users:{page - 1}"))
            if page < total_pages:
                navigation_buttons.append(types.InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"admin:users:{page + 1}"))
        ...
        subscription_status = "Активна" if subscriptions and subscriptions[-1]["end_date"] > time.time() else "Не активна"
...
<b>На сумму:</b> <code>{total_spent}₽</code>
        ...
        user_actions = [
            [types.InlineKeyboardButton(text="🚫 Заблокировать", callback_data=f"user_action:block:{user_id}") if user["level"] >= 0 else types.InlineKeyboardButton(text="⭕ Разблокировать", callback_data=f"user_action:unblock:{user_id}")],
            [types.InlineKeyboardButton(text="🔋 Выдать подписку", callback_data=f"user_action:give_subscription:{user_id}") if subscription_status == "Не активна" else types.InlineKeyboardButton(text="🪫 Забрать подписку", callback_data=f"user_action:remove_subscription:{user_id}")],
            [types.InlineKeyboardButton(text="👤 Сделать пользователем", callback_data=f"user_action:make_user:{user_id}")],
            [types.InlineKeyboardButton(text="🛍 Сделать партнёром", callback_data=f"user_action:make_partner:{user_id}")],
...
{"Ознакомиться с ассортиментом\n" + partner["partner_url"] + "\n" if partner["partner_url"] else ""}<blockquote>{partner["partner_legacy_text"]}</blockquote>
        ...
        partner_actions = [
            ...
            [types.InlineKeyboardButton(text="🆔 Указать ID партнёра", callback_data=f"partner_action:set_owner:{partner_id}")],
            [types.InlineKeyboardButton(text="👁 Скрыть", callback_data=f"partner_action:hide:{partner_id}") if partner['partner_enabled'] else types.InlineKeyboardButton(text="👁 Показать", callback_data=f"partner_action:show:{partner_id}")],
            [types.InlineKeyboardButton(text="✏ Изменить текст", callback_data=f"partner_action:edit_text:{partner_id}")],
            [types.InlineKeyboardButton(text="✏ Изменить название", callback_data=f"partner_action:edit_name:{partner_id}")],
            [types.InlineKeyboardButton(text="✏ Изменить категорию", callback_data=f"partner_action:edit_category:{partner_id}")],
            [types.InlineKeyboardButton(text="✏ Изменить URL партнёра", callback_data=f"partner_action:edit_url:{partner_id}")],
        ...
        await message.edit_text(partner_info, parse_mode="html", reply_markup=partner_markup, disable_web_page_preview=True)
    ...
    if c.data.startswith("partner_action:"):
        ...
        if action == "delete":
            ...
            await message.edit_text("<b>Введите ID нового владельца партнёра:</b>", parse_mode="HTML", reply_markup=control_partner_markup)
            await state.update_data(message_to_delete=message)
            await state.update_data(partner_id=partner_id)
            await state.set_state(PartnerForm.awaiting_partner_owner)
        
        elif action == "edit_text":
            await message.edit_text("<b>Введите новый текст партнёра:</b>", parse_mode="HTML", reply_markup=control_partner_markup)
            await state.update_data(message_to_delete=message)
            await state.update_data(partner_id=partner_id)
            await state.set_state(PartnerForm.awaiting_partner_text)
        
        elif action == "edit_name":
            await message.edit_text("<b>Введите новое название партнёра:</b>", parse_mode="HTML", reply_markup=control_partner_markup)
            await state.update_data(message_to_delete=message)
            await state.update_data(partner_id=partner_id)
            await state.set_state(PartnerForm.awaiting_partner_name)
        
        elif action == "edit_category":
            categories_text = "\n".join([f"<code>{category['category_id']}</code>: <b>{category['category_name']}</b>" for category in await db.get_categories()])
            await message.edit_text(categories_text + "\n\n<b>Введите новую категорию партнёра:</b>", parse_mode="HTML", reply_markup=control_partner_markup)
            await state.update_data(message_to_delete=message)
            await state.update_data(partner_id=partner_id)
            await state.set_state(PartnerForm.awaiting_partner_category)
        
        elif action == "edit_url":
            await message.edit_text("<b>Введите новый URL партнёра:</b>", parse_mode="HTML", reply_markup=control_partner_markup)
            await state.update_data(message_to_delete=message)
            await state.update_data(partner_id=partner_id)
...
def register_handlers(dp: Dispatcher):
    ...
    dp.message.register(add_partner_handler, PartnerForm.awaiting_partner_name_new)
    dp.message.register(add_partner_category_handler, PartnerForm.awaiting_partner_category_new)
    dp.message.register(set_partner_owner, PartnerForm.awaiting_partner_owner)
    dp.message.register(edit_partner_text, PartnerForm.awaiting_partner_text)
    dp.message.register(edit_partner_name, PartnerForm.awaiting_partner_name)
    dp.message.register(edit_partner_category, PartnerForm.awaiting_partner_category)
    dp.message.register(edit_partner_url, PartnerForm.awaiting_partner_url)
